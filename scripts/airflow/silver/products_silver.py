...
def build_products_silver():
    setup_logging()
    logging.info("Iniciando Silver - products")

    base_path = get_base_path()

    client_json = f"{base_path}/credentials/gcp_service_account.json"
    path_yaml = f"{base_path}/schemas/silver/products.yaml"

    df_products_silver = validation_coluns(
    ...
    write_silver_bigquery(
        client_json=client_json,
        df_gcp=df_products_silver,
        func=read_yaml,
        path_yaml=path_yaml,
    )

    logging.info("Silver - products finalizado com sucesso")
