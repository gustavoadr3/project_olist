...
def build_sellers_silver():
    setup_logging()
    logging.info("Iniciando Silver - sellers")

    base_path = get_base_path()

    client_json = f"{base_path}/credentials/gcp_service_account.json"
    path_yaml = f"{base_path}/schemas/silver/sellers.yaml"

    df_sellers_silver = validation_coluns(
    ...
    write_silver_bigquery(
        client_json=client_json,
        df_gcp=df_sellers_silver,
        func=read_yaml,
        path_yaml=path_yaml,
    )

    logging.info("Silver - sellers finalizado com sucesso")
